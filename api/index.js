// In-memory cache for 500 errors
...
export default async function handler(req, res) {
  ...
  try {
    ...
    function parseArticleBlock(blockHtml) {
      ...
      const dateMatch = /<h3[\s\S]*?<\/h3>\s*<span[^>]*>(

[^<]+)<\/span>/i.exec(blockHtml);
    ...
    // Additional section: Latest Releases (assumed structure similar to existing sections)
    ...
    if (latestMatch) {
      ...
      for (const artHtml of latestArticles) {
        const item = parseArticleBlock(artHtml);
        if (item.title && item.url) latestItems.push(item);
      }
